...
public class ClasssController {
    ...
    @ApiOperation(value = "Get classs page")
    @GetMapping("/page/{page}/size/{size}")
    public ResponseEntity<Page<Classs>> getByPageAndSize(
            @PathVariable Integer page,
            @PathVariable Integer size
    ) {
        return ResponseEntity.ok(classsService.getAllByPageAndSize(page, size));
    }
}
